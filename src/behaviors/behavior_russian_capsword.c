...
#define DT_DRV_COMPAT zmk_behavior_russian_capsword
...
static bool caps_word_is_alpha(uint8_t usage_id) {
    return (usage_id >= HID_USAGE_KEY_KEYBOARD_A && usage_id <= HID_USAGE_KEY_KEYBOARD_Z) || (usage_id == HID_USAGE_KEY_KEYBOARD_LEFT_BRACKET_AND_LEFT_BRACE) || (usage_id == HID_USAGE_KEY_KEYBOARD_RIGHT_BRACKET_AND_RIGHT_BRACE) || (usage_id >= HID_USAGE_KEY_KEYBOARD_SEMICOLON_AND_COLON && usage_id <= HID_USAGE_KEY_KEYBOARD_PERIOD_AND_GREATER_THAN);
}
